...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("true_label", ASMInstruction::Label); // main 函数标签
    codegen->append_inst("addi.w $t0, $zero, 1");
    codegen->append_inst("st.b",
                         {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("b exit_label");

    codegen->append_inst("false_label", ASMInstruction::Label); // main 函数标签
    codegen->append_inst("st.b",
                         {"$zero", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("b exit_label");

    codegen->append_inst("exit_label", ASMInstruction::Label);
    ...
    codegen->append_inst("sltui $t0, $t0, -1");
    codegen->append_inst("st.b",
